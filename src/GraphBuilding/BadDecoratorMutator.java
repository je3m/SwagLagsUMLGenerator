...
public class BadDecoratorMutator implements IUserGraphMutator {
	private List<FieldReader> fr;
	private List<MethodReader> mr;

	public BadDecoratorMutator(List<FieldReader> fr, List<MethodReader> mr) {
		this.fr = fr;
		this.mr = mr;
	}
	...
	private GraphVizEdge makeEdge(IEdge e){
		...
		code += ", label=\"<<THIS DECORATOR IS GARBAGE YOU SHOULD BE ASHAMED>>\"];\n";
				...
				for(FieldNode f : (List<FieldNode>) e.getTail().fields){
					if(Utilities.getClassPath(Type.getType(f.desc)).equals(e.getHead().name)){
						...
						if(!isGood){
							...
							nodesToAdd.add(this.makeNode(e.getIHead(), "BAD component"));
							nodesToAdd.add(this.makeNode(e.getITail(), "BAD decorator"));
							...
							for(IEdge e1 : g.getEdges()){
								if((e1.getDescription().contains("extends") ||
									...
									nodesToAdd.add(this.makeNode(e1.getITail(), "BAD decorator"));
								}
							}
